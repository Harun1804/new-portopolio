                            ...
                            <table class="table table-hover table-striped mb-0">
                                <thead>
                                    ...
                                    <tr>
                                        ...
                                        <th>JUMLAH YANG MEMILIKI SOSIAL MEDIA INI</th>
                                        <th>ACTION</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    ...
                                    <tr>
                                        <td class="text-bold-500">{{ ucfirst($sosmed->name ) }}</td>
                                        <td class="text-bold-500"><i class="{{ $sosmed->icon }}"></i></td>
                                        <td class="text-bold-500">{{ $sosmed->users_count }}</td>
                                        <td>
                                            <button type="button" class="btn btn-sm btn-warning d-inline text-white" wire:click="edit({{ $sosmed->id }})">Edit</button>
                                            <form method="post" wire:submit.prevent="confirm({{ $sosmed->id }})" class="d-inline">
                                                @csrf
